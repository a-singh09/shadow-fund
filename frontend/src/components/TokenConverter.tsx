...
const TokenConverter: React.FC = () => {
  ...
  // Get the encrypted balance hook with the token address
  // We must call this unconditionally due to React hooks rules
  const encryptedBalanceHook = useEncryptedBalance
    ? useEncryptedBalance(CONTRACTS.CONVERTER.ERC20)
    : null;

  // Only use the functions if we're properly registered and initialized
  const deposit =
    isRegistered && isInitialized && encryptedBalanceHook
  ...
  const handleConvert = async () => {
    ...
    if (!isRegistered) {
      toast({
        ...
        description: "Please register with eERC20 first.",
        variant: "destructive",
      });
      return;
    }
    ...
    try {
      ...
      // The eERC SDK requires messages to be null-terminated
